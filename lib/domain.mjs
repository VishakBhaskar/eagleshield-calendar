...
export function computeCellState({
  ...
  settings,
  now,
  providerOpenSeats = /** @type {number | undefined} */ (undefined),
}) {
  ...
  const locallyOpenLaneIds = territoryLanes
    .map((lane) => lane.id)
    .filter((laneId) => !blockedLaneIds.includes(laneId) && !usedLaneIds.has(laneId));
  const providerClosed = providerOpenSeats !== undefined && providerOpenSeats <= 0;
  const openLaneIds = providerOpenSeats === undefined
    ? locallyOpenLaneIds
    : locallyOpenLaneIds.slice(0, Math.max(0, providerOpenSeats));
  ...
  const openBookable = cutoff || providerClosed ? 0 : openLaneIds.length;
  return {
    capacity: territoryLanes.length,
    booked: territoryAppointments,
    blockedLaneIds,
    openLaneIds: cutoff || providerClosed ? [] : openLaneIds,
    freeRepIds: cutoff || providerClosed ? [] : freeRepIds,
    openBookable,
